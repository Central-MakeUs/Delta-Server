...
public final class ReportAggregator {
	...
	public static ReportAggregate aggregate(List<ProblemUnitStatsRow> unitStats, List<ProblemTypeStatsRow> typeStats) {
		...
		return new ReportAggregate(total, solved, unsolved, rankWeakUnits(unitStats), rankWeakTypes(typeStats));
	}

	private static List<WeakArea> rankWeakUnits(List<ProblemUnitStatsRow> unitStats) {
		return rankWeakAreas(unitStats.stream()
			.map(r -> weakArea(r.unitId(), r.unitName(), r.unsolvedCount(), r.totalCount())));
	}

	private static List<WeakArea> rankWeakTypes(List<ProblemTypeStatsRow> typeStats) {
		return rankWeakAreas(typeStats.stream()
			.map(r -> weakArea(r.typeId(), r.typeName(), r.unsolvedCount(), r.totalCount())));
	}

	private static List<WeakArea> rankWeakAreas(Stream<WeakArea> areas) {
		return areas
