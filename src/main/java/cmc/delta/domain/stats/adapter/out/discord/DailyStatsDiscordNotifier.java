...
public class DailyStatsDiscordNotifier {
	...
	private static final DateTimeFormatter TIMESTAMP_FORMATTER = DateTimeFormatter.ofPattern("yyyy년 MM월 dd일 HH:mm");
	private static final DateTimeFormatter DATETIME_FORMATTER = DateTimeFormatter.ofPattern("MM월 dd일 HH:mm");
	...
	private static final String HEADER_FORMAT = "# 잘 들으세요. %s 세모 리포트에요.\n"
		+ "근거 없이 자신을 믿으세요. 근데 숫자는 거짓말 안 해요.\n";

	private static final String FOOTER = "끝까지 가는 사람이 무조건 이겨요. 우사인볼트, 우사인볼트가 왜 빠른지 알죠?";

	private static final String TOTAL_USERS_FORMAT = "> 현재까지 총 **%d명**이 가입했어요. 탈퇴한 사람은 **%d명**이에요.\n";

	private static final String PERIOD_HEADER_FORMAT = "**%s** `%s ~ %s` — %s";

	private static final String TODAY_NEW_USERS_FORMAT = "> 신규 가입자 **%d명**. 환영은 니들이 해요. 난 바빠요.";
	private static final String TODAY_SCANS_FORMAT = "> 문제 스캔 **%d번**. 찍는다고 실력 느는 거 아닌 거 다들 알죠?";
	private static final String TODAY_WRONG_ANSWER_CARDS_FORMAT = "> 오답카드 **%d개**. 틀린 거 인정하는 건 좋아요. 근데 외워요.";
	private static final String TODAY_AI_SOLUTION_FORMAT = "> AI 풀이 **%d번**. 존나 웃기네. 귀엽네. 근데 스스로도 좀 생각해요.";

	private static final String LAST_3_DAYS_NEW_USERS_FORMAT = "> 신규 가입자 **%d명**. 근데 얼마나 남을지는 두고 봐야죠.";
	private static final String LAST_3_DAYS_SCANS_FORMAT = "> 문제 스캔 **%d번**. 3일 동안 이게 다예요? 더 할 수 있잖아요.";
	private static final String LAST_3_DAYS_WRONG_ANSWER_CARDS_FORMAT = "> 오답카드 **%d개**. 만들고 안 보면 의미 없어요. 제가 다 알아요.";
	private static final String LAST_3_DAYS_AI_SOLUTION_FORMAT = "> AI 풀이 **%d번**. 선빵은 항상 실력이에요. AI가 쳐줄 것 같아요?";

	private static final String LAST_7_DAYS_NEW_USERS_FORMAT = "> 신규 가입자 **%d명**. 일주일 기준이에요. 성장하고 있어요, 아직은.";
	private static final String LAST_7_DAYS_SCANS_FORMAT = "> 문제 스캔 **%d번**. 일주일 합산이에요. 내가 할 수 있다고 했죠?";
	private static final String LAST_7_DAYS_WRONG_ANSWER_CARDS_FORMAT = "> 오답카드 **%d개**. 이 숫자가 실력이에요. 변명은 필요 없어요.";
	private static final String LAST_7_DAYS_AI_SOLUTION_FORMAT = "> AI 풀이 **%d번**. 7일 동안 이만큼 물어봤어요. 내가 또 증명해내는 거 봤어요?";
	...
	private void postWithImage(String message) throws IOException {
		ClassPathResource resource = resolveImageResource();

		try (InputStream imageStream = resource.getInputStream()) {
			byte[] imageData = imageStream.readAllBytes();

			MultiValueMap<String, Object> multipartBody = buildMultipartBody(message, imageData,
				resource.getFilename());

			restTemplate.postForEntity(statsWebhookUrl,
				buildMultipartRequest(multipartBody), String.class);
		}
	}

	private ClassPathResource resolveImageResource() {
		...
		if (!resource.exists()) {
			return new ClassPathResource(SWINGS_IMAGES[0]);
		}
		return resource;
	}

	private MultiValueMap<String, Object> buildMultipartBody(String message, byte[] imageData, String filename)
		throws IOException {
		...
		fileHeaders.setContentDispositionFormData(DISCORD_FIELD_FILE, filename);
	...
	private HttpEntity<MultiValueMap<String, Object>> buildMultipartRequest(
		...
		return new HttpEntity<>(multipartBody, requestHeaders);
	}
}
