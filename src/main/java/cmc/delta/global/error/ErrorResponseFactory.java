...
public class ErrorResponseFactory {
	...
	public ApiResponse<Object> validationError(
		ErrorCode errorCode, MethodArgumentNotValidException ex) {
		Map<String, String> fieldErrors = extractFieldErrors(ex);
		return create(
			errorCode, errorCode.defaultMessage(), Map.of(DATA_KEY_FIELD_ERRORS, fieldErrors));
	}

	public ApiResponse<Object> constraintViolationError(
		ErrorCode errorCode, ConstraintViolationException ex) {
	...
	private Map<String, String> extractFieldErrors(MethodArgumentNotValidException ex) {
		return ex.getBindingResult().getFieldErrors().stream()
			.collect(
				Collectors.toMap(FieldError::getField, this::resolveFieldErrorMessage, (a, b) -> a));
	}
